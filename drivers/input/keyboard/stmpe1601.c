				    ...
				    const struct i2c_device_id *id)
{
	...
	int rc					= -ENOMEM;
	int device_PRODUCT_ID, device_PHASE_ID ;
...
//add read HWID for Domino Series, --
//Vince modified for DMQ PR2+++++
	if (device_PRODUCT_ID == Project_DMQ && device_PHASE_ID >= Phase_PR2)
	{
...
//	st1601_rd->early_suspend.level		= EARLY_SUSPEND_LEVEL_DISABLE_QWERTY_KEYPAD; // jjj
	...
	//Initialize GPIO
	//vince, for DMQ power consumption 2010/12/02+++
	 GPIO_CFG(90, 0, GPIO_OUTPUT, GPIO_PULL_UP, GPIO_2MA) ;//reset pin
	 GPIO_CFG(91, 0, GPIO_INPUT, GPIO_NO_PULL, GPIO_2MA) ;	//interrupt pin
	//vince 2010/12/02---
