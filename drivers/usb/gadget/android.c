...
/* [FXX_CR], Porting 4115 to 4215 */
	...
	{
		.product_id         = 0x2DE6, //c000
		.functions	    = 0x6142, /* 011111 */
		.adb_product_id     = 0x2DE6,
		.adb_functions	    = 0x6142,
	},
	// 0xC001 composition is removed diag port
	{
		.product_id         = 0x2DE8, //0xc001
		.functions	    = 0x12, /* 011101 */
		.adb_product_id     = 0x2DE8,
		.adb_functions	    = 0x12,
	},	
...
/* [FXX_CR], enable USB port dynamically */
	...
	{
		.product_id         = 0x2DE7, //0xc004
		.functions	    = 0x02, /* 100100 */
		.adb_product_id     = 0x2DE7,
		.adb_functions	    = 0x02,
	},
#if 1	
...
/* } FIH, WilsonWHLee, 2009/09/21 */
	/* FIH, chandler_usbtethering 10.8.20 +++ */
	{
		.product_id         = 0x2DE9, //c008
		.functions	    = 0x1A, /* 100100 */
		.adb_product_id     = 0x2DE9,
		.adb_functions	    = 0x1A,
	},
	/* FIH, chandler_usbtethering 10.8.20 --- */
#endif
...
static int  android_bind(struct usb_composite_dev *cdev)
{
	...
	if (0)//(gadget->ops->wakeup) && (dev->functions != ANDROID_MSC))
...
*/
	...
	if (isMoto && pid != 0xc002){
		...
		pdata->compositions   = usb_func_composition;
	    pdata->num_compositions = ARRAY_SIZE(usb_func_composition);
	}
//WilsonWHLee is Moto ? --

///WilsonWHLee 2010/02/23 5110 porting ------------
    ret = proc_comm_read_uuid((unsigned *)uuid);
    printk("proc_comm_read_uuid uuid[0]=%d,uuid[1]=%d,uuid[2]=%d,uuid[3]=%d\n",uuid[0],uuid[1],uuid[2],uuid[3]);
    if( uuid[0] != 0 || uuid[1] != 0 || uuid[2] != 0 || uuid[3] != 0){
    	if(!ret){
    		// convert hex to ascii
    		for( i=0 ; i<16; i++)
    	    {
    	    	uuid_str[((i)*2)+1] = uuid[i] & 0x0F;
    	    	uuid_str[((i)*2)] = (uuid[i] & 0xF0) >> 4;
    	    	//printk("uuid[i]=%d,uuid[(i+1]=%d\n",uuid[i],uuid[i+1]);
    		    if( uuid_str[(i)*2] >= 0 && uuid_str[(i)*2] <= 9 )
    		    	uuid_str[(i)*2] += 48; //convert 0x0~0x9
    		    else if( uuid_str[(i)*2] >= 0xA && uuid_str[(i)*2] <= 0xF )
    		        uuid_str[(i)*2] += 55; //convert 0xA~0xF
    		    if( uuid_str[((i)*2)+1] >= 0 && uuid_str[((i)*2)+1] <= 9 )
    		    	uuid_str[((i)*2)+1] += 48;
    		    else if( uuid_str[((i)*2)+1] >= 0xA && uuid_str[((i)*2)+1] <= 0xF )
    		        uuid_str[((i)*2)+1] += 55;     	
    		}
			printk("%s: uuid_str =%s\n", __func__, uuid_str);
    		memcpy(pdata->serial_number, uuid_str, 32+1);
    		//pdata->serial_number = uuid_str;
    		printk("%s: pdata->serial_number =%s\n", __func__, pdata->serial_number);
    	}
    }
    else
    	printk("%s: uuid is invalid\n",__func__);
	...
	strlcpy(serial_number , pdata->serial_number , strlen(pdata->serial_number)+1); //porting 6030 Wilson ++
	if (pid == 0xC001 )
	{
	    if( isMoto){
		    pid = 0x2DE7; 
