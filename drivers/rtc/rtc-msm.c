...
//Michael add for Power off alarm+++
static ssize_t RTC_Set_time(struct device *dev, struct device_attribute *attr, char *buf, size_t count)
{
	unsigned time_set;
...
DEVICE_ATTR(rtcset, 0666, NULL, RTC_Set_time);
//Michael add for Power off alarm---
static int
...
msmrtc_suspend(struct platform_device *dev, pm_message_t state)
{
	...
	int RPC_wakeup_cycle_time=600;  /*FIHTDC, MayLi adds for battery updates per 10 minutes in suspend mode, 2010.10.27 */
	...
	if (rtcalarm_time) 
	{
		...
		/*FIHTDC, MayLi adds for battery updates per 10 minutes in suspend mode, 2010.10.27 {+*/
