...
typedef struct  
{
	...
	int tsi_irq;
	int running;
#if defined(CONFIG_PM) && defined(CONFIG_TARGET_LOCALE_LTN_L)	//latin_cam:kihyung.nam, power management operation added
...
void s3c_tsi_set_gpio(void)
{
	...
	for(i=2;i< 7; i++)
	{
#if defined(CONFIG_TARGET_LOCALE_LTN_L)  // CYS_ 2010.05.07
...
#endif  //defined(CONFIG_TARGET_LOCALE_LTN_L)
...
#if defined(CONFIG_TARGET_LOCALE_LTN_L) // Fix the TSI data problem (Don't generated waking up sleep state)
...
#endif  //defined(CONFIG_PM) && defined(CONFIG_TARGET_LOCALE_LTN_L)
