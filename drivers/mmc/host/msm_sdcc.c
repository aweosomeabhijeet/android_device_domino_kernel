...
msmsdcc_resume(struct platform_device *dev)
{
	...
	printk(KERN_INFO "%s: host->pdev_id:[%d]\n",__func__,host->pdev_id); 
#ifdef CONFIG_MMC_AUTO_SUSPEND
...
////////////////////////////////////////////////////////////////////////
// ++ Monky: porting from 6040.
// Be ware of unknown crash here!
// Some following codes are marked out because of WIFI suspend/resume issue
// Makred code are probably related to Qualcomm's WIFI chip, just don't care.
        ...
        if (host->pdev_id == FIH_SD_SLOT) {
			printk(KERN_INFO "Mark below codes based on 6038\n");		
#ifndef CONFIG_FIH_FXX
			if (!mmc->card || mmc->card->type != MMC_TYPE_SDIO)
				mmc_resume_host(mmc);
		    if (host->plat->status_irq)
