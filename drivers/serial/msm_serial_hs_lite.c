				  ...
				  pm_message_t state)
{
	...
	if (port) {
		...
		/* Fih, Chandler, close uart clock only when console_suspend is enabled */
		if (is_console(port)&& console_suspend_enabled){
			struct msm_hsl_port *msm_hsl_port = UART_TO_MSM(port);			
			//struct clk *uart2dm_clk;
			printk("uart_debug:%s(), it's console\n",__func__);			
			msm_hsl_port->clk->count=1;	
			msm_hsl_deinit_clock(port);
		}
	}
