...
#include <linux/st1332_i2c.h>  //Added for capacitive touch panel, StanleyCheng, [DMQ_porting]
#endif
...
/* FIH, WeiChihChen, 2010/04/28{ */
	...
	#define MSM_PMEM_MDP_SIZE	0xE00000 // jones enlarge for camera video encoder
    //For HVGA camera preview & SOC sensor (gain ~2 MB)
    //HVGA (420 x 320) preview + 5M (2592 x 1944) raw data + 512 x 384 thumbnail data
    #define MSM_PMEM_ADSP_SIZE	0xC00000 // 0x156000 + 0x73D000 + 0x48000
...
/* FIH, Neo Chen, 2010/10/28, DominoQ button LEDs{ */
...
/* FIH, Neo Chen, 2010/10/27, DMP sc668 keypad LEDs{ */
...
#ifdef CONFIG_FIH_FXX
//Added the compile option for DOMINO+ WCDMA (2010.11.15)++
#ifdef CONFIG_FIH_PROJECT_DMP
...
#if 0
#ifdef CONFIG_S5K5CAG 	// DMQ CAM++, jones
	{
		I2C_BOARD_INFO("s5k5cag", 0x3C),
	},
#endif					// DMQ CAM--, jones
...
/* for DTV support, FIH, Chandler, 11.01.31 {*/
		{
...
static void config_keypad_gpios(void)
{
   ...
   if(productHWID==Project_DPD || productHWID==Project_DMP || productHWID==Project_DMQ || productHWID==Project_DQE || productHWID==Project_DQD)
   {
   ...
   else if( productHWID==Project_DMQ || productHWID==Project_DQE || productHWID==Project_DQD || productHWID==Project_DMO || productHWID==Project_DMT)
   {
...
static void msm_sdcc_setup_gpio(int dev_id, unsigned int enable)
{
	...
	printk(KERN_ERR "%s(%d): start msm_sdcc_setup_gpio -----\n", __func__, __LINE__);
...
// Original Code
    ...
    printk(KERN_ERR "%s(%d): end msm_sdcc_setup_gpio -----\n", __func__, __LINE__);
}
...
static uint32_t msm_sdcc_setup_power(struct device *dv, unsigned int vdd)
{
	...
	/* FIH, SquareCHFang, 2011/01/04 { */
	/* [DMQ.B-1016], 7627 SD card power supply source changed */
#ifdef CONFIG_FIH_FXX
	if (vreg_mmc == NULL)
	{
		int iHwid = 0;
...
{
    int rc; 
    printk(KERN_INFO "ti_wifi_power_init()\n");
...
/* } FIH, PeterKCTseng, @20090521 */
	...
	if(fih_read_new_hwid_mech_from_orighwid())
        ...
        if(project_HWID==Project_DPD || project_HWID==Project_DMP || project_HWID==Project_AI1D || project_HWID==Project_AI1S)
	{
	...
	else if(project_HWID==Project_DMQ || project_HWID==Project_DQE)
	{
		platform_add_devices(devices_S5K5CAG, ARRAY_SIZE(devices_S5K5CAG));
	}
...
//register keypad driver
	if(project_HWID==Project_AI1D || project_HWID==Project_AI1S || project_HWID==Project_DMQ || project_HWID==Project_DQE)
	{
