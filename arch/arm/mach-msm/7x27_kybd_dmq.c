...
static int EnableKeyInt = 1;// jones test 0; // 1:enable key interrupt
//-FIH_Misty
#endif
...
//vince 2011/01/20++
static int ig_volumedown_irq=0;
struct Q7x27_kybd_record *g_kbdrec=NULL;
#define MAX_LOOP_VOLUMEDOWN_IRQ 10
static pid_t volumedown_thread_id;
DECLARE_COMPLETION(volumedown_detection_comp);
static int volumedown_detection_thread(void *arg)
{
	int loop_volumedown_irq,i_volumedown_irq;//,state;
	bool volumedown_val,state;
	fih_printk(Q7x27_kybd_debug_mask, FIH_DEBUG_ZONE_G0,"7x27_kybd: volumedown_val_detection_thread running\n");
	while (1) {
		ig_volumedown_irq=0;
		loop_volumedown_irq=0;
		i_volumedown_irq=0;
		wait_for_completion(&volumedown_detection_comp);
		printk(KERN_INFO "%s: Got volumedown_detection complete signal,i_volumedown_irq=%d\n", __func__,loop_volumedown_irq);

		while(++loop_volumedown_irq<MAX_LOOP_VOLUMEDOWN_IRQ && ig_volumedown_irq!=0)
		{
			i_volumedown_irq=ig_volumedown_irq;
			msleep(10);
			if(i_volumedown_irq==ig_volumedown_irq)
			{
				disable_irq(MSM_GPIO_TO_INT(g_kbdrec->voldn_pin));
				volumedown_val= (bool)gpio_get_value(g_kbdrec->voldn_pin);
				state = (g_kbdrec->active.voldn_pin_actype == ACTIVE_HIGH) ? volumedown_val : !volumedown_val;  
	            	if (state) {
	            		input_report_key(kpdev, KEY_VOLUMEDOWN, KBD_IN_KEYPRESS);
	            		printk(KERN_INFO "FIH: by thread KEY_VOLUMEDOWN ,= %d\n", KEY_VOLUMEDOWN);
	            	} else {
	            		input_report_key(kpdev, KEY_VOLUMEDOWN, KBD_IN_KEYRELEASE); //report SELECT_INPUT_ENTER releasing
	           		printk(KERN_INFO "FIH: by thread keyrelease KEY_VOLUMEDOWN ,= %d\n", KEY_VOLUMEDOWN);
	            	}  
            		input_sync(kpdev);
            		enable_irq(MSM_GPIO_TO_INT(g_kbdrec->voldn_pin));
            		break;
			}
		}
...
/* FIH, NicoleWeng, @20100506 { */	//handle debounce time here and remove debonce time in work queue
	...
	if(lastTime!=0 && time_after(lastTime+KBD_DEBOUNCE_TIME, jiffies)){
		...
		//printk("Q7x27_kybd_irqhandler filter of, time :%d\n", jiffies_to_msecs(jiffies - lastTime)) ;
		//vince 2011/01/20++
		if (MSM_GPIO_TO_INT(kbdrec->voldn_pin) == irq)
		{
			ig_volumedown_irq++;
			if(ig_volumedown_irq==1)
			{
				g_kbdrec=kbdrec;
				complete(&volumedown_detection_comp);
			}
		}
		//vince 2011/01/20--
		return IRQ_HANDLED;
...
//+++++++++++++++++++++++++++++++FIH_F0X_misty
    if(EnableKeyInt)
    {
        ...
        // FIH, WillChen, 2009/08/14 --
            	...
            	input_sync(idev);
            			printk(KERN_INFO "FIH: input_sync KEY_VOLUMEDOWN\n");	
            			//fih_printk(Q7x27_kybd_debug_mask, FIH_DEBUG_ZONE_G0,"FIH: keypress KEY_VOLUMEDOWN\n");
            }	        
...
//		__set_bit(KEY_VOLUMEUP, idev->keybit);
		__set_bit(KEY_VOLUMEDOWN, idev->keybit);
		__set_bit(KEY_VOLUMEUP, idev->keybit);
		volumedown_thread_id = kernel_thread(volumedown_detection_thread, NULL, CLONE_FS | CLONE_FILES);//vince
#endif
...
#if ACTIVE_MODE_ENABLE // Peter, Debug
// FIH, vince 2010/11/10+++
    if (g_PRODUCTHWID  == Project_DMQ || g_PRODUCTHWID  == Project_DMT || g_PRODUCTHWID  == Project_DMO) {
	printk("FIH: Project_DMQ, Product HWID:%d\n ",g_PRODUCTHWID);
    ...
    else if (g_PRODUCTHWID == Project_DMP || g_PRODUCTHWID == Project_DPD || g_PRODUCTHWID==Project_DP2)
    {
    	rd->active.volup_pin_actype     = ACTIVE_LOW;
        rd->active.voldn_pin_actype     = ACTIVE_LOW;
  ...
  //      rd->active.center_pin_actype    = ACTIVE_LOW;
	...
	//setup_data->volup_pin;
	//setup_data->voldn_pin;
...
// FIH, vince 2010/11/10---
#if 0 //old read hwid mechanism, vince++
...
#endif//endif ACTIVE_MODE_ENABLE
/* } FIH, PeterKCTseng, @20090520 */
...
//FIH, Vince 2010/11/12+++
	//products without center keys
    if (g_PRODUCTHWID  == Project_DMQ)
    {
	g_centerkey = false ;   
    }
//FIH, Vince 2010/11/12---
...
#if CENTER_KEY_ENABLE // Peter, Debug
	if(b_center_GPIO)
		if(g_centerkey) INIT_WORK(&rd->kybd_centerkey, Q7x27_kybd_centerkey);
#endif
/* } FIH, PeterKCTseng, @20090527 */
    KeySetup();
